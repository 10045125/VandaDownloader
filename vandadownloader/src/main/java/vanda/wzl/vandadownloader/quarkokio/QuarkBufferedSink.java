...
public interface QuarkBufferedSink extends QuarkSink {
   ...
   * this sink.
   */
  QuarkBufferedSink write(byte[] source) throws IOException;

  /**
   * Like {@link OutputStream#write(byte[], int, int)}, this writes {@code byteCount}
   * bytes of {@code source}, starting at {@code offset}.
   */
  QuarkBufferedSink write(byte[] source, int offset, int byteCount) throws IOException;
   ...
   */
  long writeAll(QuarkSource source) throws IOException;

  /** Removes {@code byteCount} bytes from {@code source} and appends them to this sink. */
  QuarkBufferedSink write(QuarkSource source, long byteCount) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeUtf8(String string) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeUtf8(String string, int beginIndex, int endIndex) throws IOException;

  /** Encodes {@code codePoint} in UTF-8 and writes it to this sink. */
  QuarkBufferedSink writeUtf8CodePoint(int codePoint) throws IOException;

  /** Encodes {@code string} in {@code charset} and writes it to this sink. */
  QuarkBufferedSink writeString(String string, Charset charset) throws IOException;

  /**
   * Encodes the characters at {@code beginIndex} up to {@code endIndex} from {@code string} in
   * {@code charset} and writes it to this sink.
   */
  QuarkBufferedSink writeString(String string, int beginIndex, int endIndex, Charset charset)
      throws IOException;

  /** Writes a byte to this sink. */
  QuarkBufferedSink writeByte(int b) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeShort(int s) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeShortLe(int s) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeLong(long v) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeLongLe(long v) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeDecimalLong(long v) throws IOException;
   ...
   * }</pre>
   */
  QuarkBufferedSink writeHexadecimalUnsignedLong(long v) throws IOException;
   ...
   *   BufferedSink b1 = buffer(b0);
   *   BufferedSink b2 = buffer(b1);
   *
   *   b2.writeUtf8("hello");
