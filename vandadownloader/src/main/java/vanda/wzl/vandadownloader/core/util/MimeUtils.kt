...
fun guessFileName(url: String, contentDisposition: String): String {
    return guessFileName(url, contentDisposition, "")
}
 ...
 *
 * 中文补充,如果通过url和contentDisposition检测到文件名和后缀将不再使用mimetype校对,如果检测不到则使用mimetype校对
 *
...
fun guessFileName(
    ...
    if (filename == null && contentDisposition != null) {
        ...
        if (!TextUtils.isEmpty(filename) && filename!!.contains("attachment")) {
            filename = filename.substring(filename.lastIndexOf("attachment") + "attachment".length, filename.length)
        }
        if (filename != null) {
    ...
    if (dotIndex < 0) {
        ...
        //                String typeFromExt = guessMimeTypeFromExtension(
...
//            Pattern.compile("\\s*filename\\s*=\\s*(\"?)([^\"]*)\\1\\s*$",
 ...
 */
internal fun parseContentDisposition(disposition: String?): String? {
    //        try {
    ...
    val parser = MIMEParameterParser()
    var result = parser.parse("filename", disposition)
    if (!TextUtils.isEmpty(result)) {
        result = result!!.trim { it <= ' ' }
        result = result.replace("&amp".toRegex(), "&")
        result = result.replace("&lt".toRegex(), "<")
        result = result.replace("&quot".toRegex(), "\"")
        result = result.replace("&nbsp".toRegex(), " ")
        fixFileName(result)
    }
    return result
 ...
 */
private val UNICODE_SURROGATE_START_CHAR: Char = 0xD800.toChar()
private val UNICODE_SURROGATE_END_CHAR: Char = 0xDFFF.toChar()
private fun removeSurrogateChars(string: String): String {
...
private fun containsSurrogateChar(string: String): Boolean {
    if (TextUtils.isEmpty(string)) {
        return false
    }
    val length = string.length
    var hasSurrogateChar = false
    for (i in 0 until length) {
        val c = string[i]
        if (c in UNICODE_SURROGATE_START_CHAR..UNICODE_SURROGATE_END_CHAR) {
